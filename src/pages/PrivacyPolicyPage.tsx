...
import { contactInfo, privacyPolicyHero, privacyPolicySections } from '@/constants';
          ...
          <div className="policy-hero-content">
            ...
            <div className="max-w-3xl">
              ...
              <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-brand-blue/10 rounded-full mb-4 sm:mb-5 md:mb-6">
                <Shield className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-brand-blue" />
                <span className="text-xs sm:text-sm font-medium text-brand-blue">
                  {privacyPolicyHero.badge}
                </span>
              </div>
              ...
              <h1 className="font-display text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-brand-dark leading-[1.15] mb-3 sm:mb-4 md:mb-6">
                {privacyPolicyHero.title}{' '}
                <span className="text-brand-blue">{privacyPolicyHero.titleHighlight}</span>
              </h1>

              {/* Description */}
              <p className="text-sm sm:text-base md:text-lg text-gray-600 leading-relaxed max-w-2xl mb-3 sm:mb-4">
                {privacyPolicyHero.description}
              </p>

              {/* Last Updated */}
              <p className="text-xs sm:text-sm text-gray-500">
                Last Updated: {privacyPolicyHero.lastUpdated}
              </p>
            </div>
          </div>
        </div>
            ...
            {privacyPolicySections.map((section) => {
              ...
              return (
                <div
                  key={section.id}
                  id={section.id}
                  className="policy-section bg-white rounded-xl sm:rounded-2xl p-5 sm:p-6 md:p-8 shadow-lg shadow-gray-100/50 border border-gray-100 scroll-mt-28 sm:scroll-mt-32"
                >
                  {/* Section Header */}
                  <div className="flex items-start gap-3 sm:gap-4 mb-4 sm:mb-5 md:mb-6">
                    <div className="w-10 sm:w-11 md:w-12 h-10 sm:h-11 md:h-12 bg-brand-blue/10 rounded-lg sm:rounded-xl flex items-center justify-center flex-shrink-0">
                      <IconComponent className="w-5 sm:w-5.5 md:w-6 h-5 sm:h-5.5 md:h-6 text-brand-blue" />
                    </div>
                    <h2 className="font-display text-lg sm:text-xl md:text-2xl font-bold text-brand-dark pt-1.5 sm:pt-2">
                      {section.title}
                    </h2>
                  </div>

                  {/* Section Content */}
                  <div className="space-y-4 sm:space-y-5 md:space-y-6 pl-0 sm:pl-14 md:pl-16">
