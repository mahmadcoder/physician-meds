...
function bucketDate(dateStr: string, period: DatePeriod): string | null {
...
function generateBuckets(period: DatePeriod): string[] {
  const { start, end } = getDateRange(period);
...
export default function AnalyticsChart({
  ...
  const chartData: ChartDataPoint[] = useMemo(() => {
    ...
    contacts.forEach((c) => { const k = bucketDate(c.created_at, period); if (k && map.has(k)) map.get(k)!.contacts++; });
    consultations.forEach((c) => { const k = bucketDate(c.created_at, period); if (k && map.has(k)) map.get(k)!.consultations++; });
    ctaInquiries.forEach((c) => { const k = bucketDate(c.created_at, period); if (k && map.has(k)) map.get(k)!.ctaInquiries++; });
    comments.forEach((c) => { const k = bucketDate(c.created_at, period); if (k && map.has(k)) map.get(k)!.comments++; });
    subscribers.forEach((s) => { const k = bucketDate(s.subscribed_at, period); if (k && map.has(k)) map.get(k)!.subscribers++; });

    return Array.from(map.values());
  }, [period, contacts, consultations, ctaInquiries, comments, subscribers]);
